...
for cnr in range(len(contours)):
    ...
    if area > 100:
        ...
        if factor > factorTeRond:#binnenste cirkel van liggende moer (veel te rond)
            continue
        elif factor < factorRedelijkRond and holes is 0: #liggende bout (niet rond, zonder gaten)
            cv2.drawContours(img, [cnt], -1, (0,255,255), 3)
        elif factor > factorErgRond and holes is 0: #staande bout (erg rond, zonder gaten)
            cv2.drawContours(img, [cnt], -1, (100,200,0), 3)
        elif factor > factorRedelijkRond and holes > 0:#liggende moer(redelijk rond, met gaten)
            cv2.drawContours(img, [cnt], -1, (255,0,0), 3)
        elif factor > factorRedelijkRond and holes is 0:#staande moer(redelijk rond, zonder gaten)
